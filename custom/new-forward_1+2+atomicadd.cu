...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    int m = blockIdx.z;
    int bc = blockIdx.y*T_WIDTH + threadIdx.y;
    ...
    /*bool lower_bound = (w >= 0 && h >= 0 && b >= 0);
    bool upper_bound = (w < W && h < H && b < B);
...
__host__ void conv_forward_gpu(float *host_y, const float *host_x, const float *host_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    cudaMemset(device_y, 0, B * M * (H-K+1)*(W-K+1) * sizeof(float));

    // Set the kernel dimensions and call the kernel
    dim3 dimGrid(ceil((1.0*(W-K+1)*(H-K+1))/T_WIDTH), ceil((1.0*B*C)/T_WIDTH), M);
