...
__global__ void conv_forward_kernel(float *__restrict__ y, const float * __restrict__ x, const int B, const int M, const int C, const int H, const int W, const int K, float* k)
{
    /*
...
#define k4d(i3, i2, i1, i0) k[(i3) * (C * K * K) + (i2) * (K * K) + (i1) * (K) + i0]
    ...
    int m = blockIdx.z;
    int h = blockIdx.y*T_WIDTH + threadIdx.y;
    int w = blockIdx.x*T_WIDTH + threadIdx.x;
...
__host__ void conv_forward_gpu(float *host_y, const float *host_x, const float *host_k, const int B, const int M, const int C, const int H, const int W, const int K)
{
    ...
    dim3 dimGrid(ceil((1.0*(W-K+1))/T_WIDTH), ceil((1.0*(H-K+1))/T_WIDTH), M);
